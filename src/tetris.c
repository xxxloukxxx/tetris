 ...
 * Les fonctions
 */
bool is_gameover(t_tetris_game *g) {
    return (!block_valid(*g, g->current));
}
...
int check_lines(t_tetris_game *g) {
    ...
    for(y = 0; y < YSIZE; y++) {
        line_clear = true;
        for(x = 0; x < XSIZE; x++) {
            if(g->f[x][y] == -1) line_clear = false;
        }
        if(line_clear) {
            nb_lines_clear++;
            for(yy = y; yy > 0; yy--)
                for(x = 0; x < XSIZE; x++) g->f[x][yy] = g->f[x][yy - 1];
            for(x = 0; x < XSIZE; x++) g->f[x][0] = -1;
        }
    }
...
bool drop_block(t_tetris_game *g) {
    int     x, y;
    t_block b = g->current;
    if(!block_valid((*g), b)) return false;
    ...
    for(y = 0; y < 4; y++) {
        for(x = 0; x < 4; x++) {
            if(block_xy(b, x, y)) g->f[b.x + x][b.y + y] = b.type;
        }
    }
    return true;
}
...
bool block_xy(t_block b, int x, int y) {
    int tmp;
    if(!in(x, 0, 3) || !in(y, 0, 3)) return false;
...
bool block_valid(t_tetris_game g, t_block b) {
    ...
    for(x = 0; x < 4; x++)
        for(y = 0; y < 4; y++)
            if(block_xy(b, x, y)) {
                if(!in(b.x + x, 0, XSIZE - 1) || !in(b.y + y, 0, YSIZE - 1)) return false;
                if(g.f[b.x + x][b.y + y] != (-1)) return false;
            }
    return true;
}
